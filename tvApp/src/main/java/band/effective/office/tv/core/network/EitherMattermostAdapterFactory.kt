...
class EitherMattermostAdapterFactory @Inject constructor() : CallAdapter.Factory() {
    override fun get(
        returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<MattermostErrorResponse>(
            MattermostErrorResponse::class.java,
            annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
        ...
        private val errorBodyConverter: Converter<ResponseBody, MattermostErrorResponse>
    ) :
    ...
    private class ResultCallWrapper<F, T>(
        ...
        private fun getReasonByCode(
            response: Response<T>,
            errorResponse: MattermostErrorResponse
        ) = when (response.code()) {
