...
fun SwaggerDocument.returnBookings(): OpenApiRoute.() -> Unit = {
    ...
    request {
        ...
        queryParameter<Long>("min_start_time") {
            description = "Minimum booking start time. Default value: ${BookingConstants.MIN_SEARCH_START_TIME} (${
        ...
        queryParameter<Long>("max_start_time") {
            description = "Maximum booking start time. If provided, should be greater then min_start_time."
    ...
    response {
        ...
        HttpStatusCode.BadRequest to {
            description = "Max start time isn't greater then min start time, or one of them can't be parsed to Long"
        }
        HttpStatusCode.NotFound to {
