...
object RecurrenceRuleFactory {
    fun String.getRecurrence(): Recurrence =
        substringAfter(":").split(";").fold(
            Recurrence(
                interval = 0,
                freq = Freq.DAILY,
                ending = Ending.Empty,
            ...
            when (parts.first()) {
                ...
                "UNTIL" -> recurrence.copy(ending = Ending.Until(parts.last().toLong()))
    ...
    private fun String.toDayNum(): Int =
        when (this) {
            "MO" -> 1
            "TU" -> 2
            "WE" -> 3
