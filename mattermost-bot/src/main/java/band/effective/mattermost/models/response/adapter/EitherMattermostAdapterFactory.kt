...
import band.effective.mattermost.models.error.MattermostApiError
...
class EitherMattermostAdapterFactory: CallAdapter.Factory() {
    override fun get(
            returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<MattermostApiError>(
                MattermostApiError::class.java,
                annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
            ...
            private val errorBodyConverter: Converter<ResponseBody, MattermostApiError>
    ) :
    ...
    private class ResultCallWrapper<F, T>(
        ...
        private fun getReasonByCode(
                response: Response<T>,
                errorResponse: MattermostApiError
        ) = when (errorResponse.statusCode) {
            404 -> ErrorReason.NotFound("Not found")
            119 -> ErrorReason.ServerError("Authorization error")
            else -> ErrorReason.ServerError("Error with mattermost api. Error code ${errorResponse.statusCode}")
        }
    }
}
